...
I created a synthetic benchmark where I simulated repeated random writes that we do (movement/content changes/etc) to a Yjs document with a `Y.Map`. The results were pretty terrible and it confirmed our suspicions.

Now we have identified the culprit - but _what_ could we do about it?
...
One promising avenue was Yjs' `encodeStateAsUpdateV2` method for creating the initial state vector. This was a sureshot way of reducing the document size itself as it compressed larger documents much better than the v1 implementation, trading off the efficiency in smaller `Y.Doc`s
...
### `SyncStep1` processed by server, `SyncStep2` response ↙
...
### Client processes received `SyncStep2` and finishes sync ↻
...
We released it initially under a feature flag in order to limit the blast radius to just ourselves at Chronicle, but now it has been released to everyone 😁
